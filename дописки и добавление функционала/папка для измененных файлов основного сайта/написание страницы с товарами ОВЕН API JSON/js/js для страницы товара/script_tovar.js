async function getResponse(){
    ...
    product.innerHTML += `
        <a class="link-photo" href="${content[0].items[0].products[29].image}">
            <img class="hero-photo" src="${content[0].items[0].products[29].image}" alt="ТРМ1">
        </a>
    `

    //вывод маленьких фото
    let keys = Object.keys(content[0].items[0].products[29].images);
    for(let key in keys) {
        secondary.innerHTML += `
        <div class="wrapper-link-photo">
            <a class="link-photo" data-lightbox="image-tovar-group" href="${content[0].items[0].products[29].images[key].src}">
                <img class="photos__img lazy-img" src="${content[0].items[0].products[29].images[key].src}" alt="${content[0].items[0].products[29].images[key].alt}">
            </a>
        </div>
    `
    }
    ...
    //вывод цены 
    let listPrice = Object.keys(content[0].items[0].products[29].prices);
    for(let key in listPrice) {

        modificationBlock.innerHTML += `
        <li data-name class="list-modification">
            <span data-name class="name-modification">${content[0].items[0].products[29].prices[key].name}</span>
            <span data price class="string-price" style="display: none;">${content[0].items[0].products[29].prices[key].price}</span>
        </li>
    `
    }
    ...
    desc.innerHTML += `
        ${content[0].items[0].products[29].desc}
    `
    //блок с инфой из specs(json)
    specs.innerHTML += `
        ${content[0].items[0].products[29].specs}
    `
    //блок с инфой из docs-заголовок документация
    headerDocumentation.innerHTML += `
        ${content[0].items[0].products[29].docs[0].name}
    `

    //содержание блока документация
    let doc = Object.keys(content[0].items[0].products[29].docs[0].items);
    for(let key in doc) {
        ...
        <div class="wrapper-block-img-doc">
            ...
            <div class="block-name-certificates">
                <a class="link-doc" href="${content[0].items[0].products[29].docs[0].items[key].link}">
                    ${content[0].items[0].products[29].docs[0].items[key].name}
                </a><br>
            </div>
        </div>
    `
    ...
    headerCertificates.innerHTML += `
        ${content[0].items[0].products[29].docs[1].name}
    `

    //содержание блока сертификаты
    let certificates = Object.keys(content[0].items[0].products[29].docs[1].items);
    for(let key in certificates) {
        ...
        <div class="wrapper-block-img-doc">
            ...
            <div class="block-name-certificates">
                <a class="link-doc" href="${content[0].items[0].products[29].docs[1].items[key].link}">
                    ${content[0].items[0].products[29].docs[1].items[key].name}
                </a><br>
            </div>
        </div>
    `
    ...
    //блок с инфой из docs-заголовок ПО
        headerPo.innerHTML += `
                ${content[0].items[0].products[29].docs[2].name}
        `

        //содержание блока ПО
        let keysPo = Object.keys(content[0].items[0].products[29].docs[2].items);
        for(let key in keysPo) {
            ...
            <div class="wrapper-block-img-doc">
                ...
                <div class="block-name-po">
                  <a class="link-doc" href="${content[0].items[0].products[29].docs[2].items[key].link}">
                    ${content[0].items[0].products[29].docs[2].items[key].name}
                  </a><br>
                </div>
            </div>
        `
    ...
    let keysPrice = Object.keys(content[0].items[0].products[29].prices);
    for(let key in keysPrice) {

        wrapperPrice.innerHTML += `
        <div class="wrapper-price_block">
            <div class="name-prodict">${content[0].items[0].products[29].prices[key].name}</div>
            <div class="price-prodict">${content[0].items[0].products[29].prices[key].price} ₽</div>
        </div>
    `
    }

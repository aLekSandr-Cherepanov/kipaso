async function getResponse(){
    ...
    header.innerHTML += `
        ...
        <h2>${content[0].items[0].products[1].name}</h2>    
    ...
    product.innerHTML += `
        <div>
            <img class="style-photo" src="${content[0].items[0].products[1].image}">
        </div>
    `
    ...
    for(let key in keys) {
        ...
        secondary.innerHTML += `
        <div>
            <img src="${content[0].items[0].products[1].images[key].src}">
        </div>
    `
    }
    ...
    //вывод цены
    price.innerHTML += `
            <span> Цена:${content[0].items[0].products[1].prices[1].price}</span>
    `

    //блок с инфой из desc(json)
    desc.innerHTML += `
        <div>
            <p>${content[0].items[0].products[1].desc}</p>
        </div>
    `
    ...
    for(let key in doc) {
        ...
        <a href="${content[0].items[0].products[1].docs[0].items[key].link}">
            <span>${content[0].items[0].products[1].docs[0].items[key].name}</span>
        </a><br>
    `
    }
    ...
    for(let key in certificates) {
        ...
        <a href="${content[0].items[0].products[1].docs[1].items[key].link}">
            <span>${content[0].items[0].products[1].docs[1].items[key].name}</span>
        </a><br>
    `
    }
    ...
    function blockPo(){
        ...
        for(let key in keysPo) {
            ...
            contentPo.innerHTML += `
            <a href="${content[0].items[0].products[1].docs[2].items[key].link}">
                <span>${content[0].items[0].products[1].docs[2].items[key].name}</span>
            </a><br>
        `
        }
    }
    ...
    for(let key in keysPrice) {
        ...
        <div>${content[0].items[0].products[1].prices[key].name}</div>
        <div>${content[0].items[0].products[1].prices[key].price}</div>
    `
    }
